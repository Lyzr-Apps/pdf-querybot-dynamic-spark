...
const MAGIC_BYTES: Record<string, number[]> = {
  ...
  // text/plain has no magic bytes - we'll validate it differently
}

/**
 * Validate that a file's content matches its declared MIME type.
 * This prevents content-type spoofing where a malicious file
 * could be uploaded disguised as a safe type.
 */
async function validateFileMagicBytes(
  ...
  // Text files have no magic bytes - just check they're reasonably small
  // and contain valid UTF-8 text
  if (!expectedMagic) {
    if (file.type === 'text/plain') {
      // Limit text file size to prevent abuse
      if (file.size > 10 * 1024 * 1024) {
        // 10MB limit
        return { valid: false, error: 'Text file too large (max 10MB)' }
      }
      ...
      try {
        ...
        if (text.includes('\0')) {
          return {
            valid: false,
            error: 'File contains binary content, not plain text',
          }
        }
        return { valid: true }
      } catch {
  ...
  const matches = expectedMagic.every((b, i) => bytes[i] === b)
